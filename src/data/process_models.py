...
def save_samples(
    ...
    output_path = target_dir / relative_path.with_name(
        f"{relative_path.stem}_{suffix}.hdf5"
    )
...
def split_samples(
    samples: np.ndarray, batch_size: int, train_batches: int, num_batches: int
):
...
def process_models(
    ...
    with tqdm(total=len(paths), desc="Processing files") as pbar:
        for model_file in paths:
            ...
            training_batches, validation_batches = split_samples(
                samples, batch_size, train_batches, num_batches
            )
            save_samples(
                training_batches,
                context_size,
