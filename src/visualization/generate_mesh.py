...
def _calculate_spacing(x: np.ndarray, y: np.ndarray, z: np.ndarray) -> Tuple[float, float, float]:
    return x[1] - x[0], y[1] - y[0], z[1] - z[0]


def _get_verts_faces(sdf_values: np.ndarray, spacing: Tuple[float, float, float], level: float = 0) -> Tuple[np.ndarray, np.ndarray]:
    verts, faces, normals, values = measure.marching_cubes(sdf_values, level, spacing=spacing)
    return verts, faces
...
def _generate_mesh(sdf_func: Callable[[np.ndarray], float], bounds: List[Tuple[float, float]],
    batch_size: int, resolution: int) -> Tuple[np.ndarray, np.ndarray]:
    '''
...
def _get_model_prediction(model, context: np.ndarray, input_data: np.ndarray,
    ...
    input_data_tensor = torch.tensor(input_data, dtype=torch.float32).unsqueeze(1).to(device)

    output: torch.Tensor = model.forward(context_tensor, input_data_tensor)['logits']
    return output.squeeze().cpu().numpy()


def generate_mesh(model, file_path: Path, output_path: Path, device: torch.device,
    batch_size: int, bounds: list[tuple] = [(-1.5, 1.5), (-1.5, 1.5), (-1.5, 1.5)],
    resolution: int = 200, context_size: int = 50):
    ...
    if file_path.suffix == '.obj':
        ...
        loader.close()
    sdf_func = lambda p: _get_model_prediction(model, context, p, device, batch_size=batch_size)
    with torch.no_grad():
        verts, faces = _generate_mesh(sdf_func, bounds, batch_size=batch_size, resolution=resolution)
    _save_mesh_to_obj(output_path, verts, faces)


def generate_meshes(model, obj_dir: Path, output_dir: Path, naming_format: str,
    device: torch.device, batch_size: int, resolution: int = 100, context_size: int = 50,
    ...
    for obj_path in tqdm(obj_files, desc="Processing models"):
        ...
        generate_mesh(model, obj_path, mesh_output_path, device, batch_size,
            bounds=bounds, resolution=resolution, context_size=context_size)