...
class LazySampleDataset(Dataset):
    def __init__(self, filepaths: list[str]) -> None:
        ...
        self.loaders = [LazySampleLoader(filepath=filepath) for filepath in self.filepaths]
        ...
        for loader in self.loaders:
            ...
            self.sample_indices_map.extend([(loader, batch_index, sample_index)
    ...
    def _sample_indices_map(self, index: int) -> tuple[LazySampleLoader, int, int]:
        if index < 0 or index >= self.total_samples:
            raise IndexError(f"Index {index} out of bounds for dataset of size {self.total_samples}")
